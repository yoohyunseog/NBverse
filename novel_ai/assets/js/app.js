    ...
    async function loadServerNovels() {
        try {
            ...
            try {
                ...
                addLog('error', `[BIT 기반 소설 목록] 서버에 연결할 수 없습니다: ${fetchError.message}`);
                addLog('error', '[BIT 기반 소설 목록] 서버가 실행 중인지 확인하세요. (포트 8123)');
                return;
            }
            ...
            if (!response.ok) {
                ...
                if (response.status === 0 || !response.status) {
                    addLog('error', '[BIT 기반 소설 목록] 서버에 연결할 수 없습니다. 서버가 실행 중인지 확인하세요.');
                }
                return;
            }
