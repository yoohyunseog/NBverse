...
(function() {
    ...
    function extractAttributeParts(rawAttributeText, novelTitle) {
        ...
        if (novelPrefix && attributeOnly.startsWith(novelPrefix)) {
            attributeOnly = attributeOnly.substring(novelPrefix.length);
        }
        const full = novelTitle && attributeOnly
            ? `${novelTitle} → ${attributeOnly}`
            : attributeOnly;
    ...
    async function autoSave(overrideData = null) {
        ...
        if (typeof window.addRightLog === 'function') {
            ...
            window.addRightLog('info', `[BIT 계산] 데이터 BIT (속성 텍스트 "${attributeDisplay}"의 BIT 사용): MAX=${dataBits.max ? dataBits.max.toFixed(15) : 'null'}, MIN=${dataBits.min ? dataBits.min.toFixed(15) : 'null'}`);
        }
        ...
        try {
            ...
            if (typeof window.addRightLog === 'function') {
                ...
                window.addRightLog('info', `[서버 전송] 데이터 BIT (속성 텍스트 "${attributeTextDisplay}"의 BIT 사용): MAX=${dataBits.max.toFixed(15)}, MIN=${dataBits.min.toFixed(15)}`);
                if (chapter) {
            ...
            if (result.ok) {
                if (typeof window.addRightLog === 'function') {
                    ...
                    const hasData = finalDataText && finalDataText.trim().length > 0;
                    ...
                    if (hasData && dataBits && dataBits.max !== null && dataBits.min !== null) {
                        const attributeTextDisplay = fullAttributeText.length > 50 ? fullAttributeText.substring(0, 50) + '...' : fullAttributeText;
                        window.addRightLog('success', `[자동 저장] 데이터 BIT (속성 텍스트: "${attributeTextDisplay}" 사용): MAX=${dataBits.max.toFixed(15)}, MIN=${dataBits.min.toFixed(15)}`);
