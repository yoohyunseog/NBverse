...
app.post('/api/attributes/data', (req, res) => {
  try {
    ...
    if (novelTitleBitMax !== undefined && novelTitleBitMax !== null && Number.isFinite(novelTitleBitMax)) {
      const { targetDir } = nestedPathFromNumber('max', novelTitleBitMax);
      try { 
      ...
      try { 
        ...
        console.log('[Novel] 소설 제목 MAX 폴더에 저장:', recordPath); 
        written.novelTitleMax = recordPath; 
      } catch (e) { 
        console.warn('[Novel] 소설 제목 MAX 저장 실패:', recordPath, e); 
      }
    }
    
    // 2. 소설 제목 BIT MIN으로 MIN 폴더에 저장
    if (novelTitleBitMin !== undefined && novelTitleBitMin !== null && Number.isFinite(novelTitleBitMin)) {
      const { targetDir } = nestedPathFromNumber('min', novelTitleBitMin);
      try { 
        fs.mkdirSync(targetDir, { recursive: true }); 
      } catch (_) {}
      const recordPath = generateRecordFilePath(targetDir);
      try { 
        fs.writeFileSync(recordPath, line, 'utf8'); 
        console.log('[Novel] 소설 제목 MIN 폴더에 저장:', recordPath); 
        written.novelTitleMin = recordPath; 
      } catch (e) { 
        console.warn('[Novel] 소설 제목 MIN 저장 실패:', recordPath, e); 
      }
    }
    
    // 3. 챕터 BIT MAX로 MAX 폴더에 저장
    if (chapterBitMax !== undefined && chapterBitMax !== null && Number.isFinite(chapterBitMax)) {
      const { targetDir } = nestedPathFromNumber('max', chapterBitMax);
      try { 
        fs.mkdirSync(targetDir, { recursive: true }); 
      } catch (_) {}
      const recordPath = generateRecordFilePath(targetDir);
      try { 
        fs.writeFileSync(recordPath, line, 'utf8'); 
        console.log('[Chapter] 챕터 MAX 폴더에 저장:', recordPath); 
        written.chapterMax = recordPath; 
      } catch (e) { 
        console.warn('[Chapter] 챕터 MAX 저장 실패:', recordPath, e); 
      }
    }
    
    // 4. 챕터 BIT MIN으로 MIN 폴더에 저장
    if (chapterBitMin !== undefined && chapterBitMin !== null && Number.isFinite(chapterBitMin)) {
      const { targetDir } = nestedPathFromNumber('min', chapterBitMin);
      try { 
        fs.mkdirSync(targetDir, { recursive: true }); 
      } catch (_) {}
      const recordPath = generateRecordFilePath(targetDir);
      try { 
        fs.writeFileSync(recordPath, line, 'utf8'); 
        console.log('[Chapter] 챕터 MIN 폴더에 저장:', recordPath); 
        written.chapterMin = recordPath; 
      } catch (e) { 
        console.warn('[Chapter] 챕터 MIN 저장 실패:', recordPath, e); 
      }
    }
    ...
    if (Number.isFinite(dataBitMax)) {
      const { targetDir } = nestedPathFromNumber('max', dataBitMax);
      try { 
        fs.mkdirSync(targetDir, { recursive: true }); 
      } catch (_) {}
      const recordPath = generateRecordFilePath(targetDir);
      try { 
        fs.writeFileSync(recordPath, line, 'utf8'); 
        console.log('[Attribute] 데이터 MAX 폴더에 저장:', recordPath); 
        written.dataMax = recordPath; 
      } catch (e) { 
        console.warn('[Attribute] 데이터 MAX 저장 실패:', recordPath, e); 
      }
    }
    
    // 8. 데이터 BIT MIN으로 MIN 폴더에 저장
    if (Number.isFinite(dataBitMin)) {
      const { targetDir } = nestedPathFromNumber('min', dataBitMin);
      try { 
        fs.mkdirSync(targetDir, { recursive: true }); 
      } catch (_) {}
      const recordPath = generateRecordFilePath(targetDir);
      try { 
        fs.writeFileSync(recordPath, line, 'utf8'); 
        console.log('[Attribute] 데이터 MIN 폴더에 저장:', recordPath); 
        written.dataMin = recordPath; 
      } catch (e) { 
        console.warn('[Attribute] 데이터 MIN 저장 실패:', recordPath, e); 
      }
    }
...
app.post('/api/attributes/data/delete', (req, res) => {
  try {
    ...
    // 3. 데이터 MAX 폴더
    if (Number.isFinite(dataBitMax)) {
      const { targetDir, nestedFile, baseDir, digits } = nestedPathFromNumber('max', dataBitMax);
      const sources = [];
      if (fs.existsSync(nestedFile)) sources.push(nestedFile);
      sources.push(...listRecordFiles(targetDir).filter(f => f !== nestedFile));
      if (sources.length === 0) {
    ...
    // 4. 데이터 MIN 폴더
    if (Number.isFinite(dataBitMin)) {
      const { targetDir, nestedFile, baseDir, digits } = nestedPathFromNumber('min', dataBitMin);
      const sources = [];
      if (fs.existsSync(nestedFile)) sources.push(nestedFile);
      sources.push(...listRecordFiles(targetDir).filter(f => f !== nestedFile));
      if (sources.length === 0) {
