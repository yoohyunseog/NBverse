...
function initializeDataDirectories() {
  ...
  ensureDirectory(CHARACTERS_DIR);
  ensureDirectory(WORLD_DIR);
  ensureDirectory(MEMORY_DIR);
  ensureDirectory(NOVELS_DIR);
  ensureDirectory(HONEYCOMB_DIR);
  ensureDirectory(HIERARCHY_DIR);
  ensureDirectory(path.join(DATA_DIR, 'max'));
  ensureDirectory(path.join(DATA_DIR, 'min'));
  ensureDirectory(path.join(DATA_DIR, 'training'));
  ensureDirectory(path.join(DATA_DIR, 'world'));
  ensureDirectory(SERVER_LOGS_DIR);
...
function resetAllTestData() {
  try {
    ...
    if (fs.existsSync(SERVER_LOGS_DIR)) {
      fs.rmSync(SERVER_LOGS_DIR, { recursive: true, force: true });
    }
  } catch (error) {
    console.error('[Reset] Failed to remove existing directories:', error);
    throw error;
  }
  initializeDataDirectories();
}
  ...
  try {
    ...
    return names.map(name => {
      ...
      return {
        name,
        path: fullPath,
        url: `/novel_ai/v1.0.4/server/logs/${name}`,
        size: stats.size,
        mtime: stats.mtime,
        ctime: stats.ctime,
        entries,
...
function generateRecordFilePath(targetDir) {
  const timestamp = Date.now();
  const random = Math.random().toString(36).slice(2, 8);
  return path.join(targetDir, `${timestamp}-${random}.ndjson`);
}
...
app.post('/api/log', (req, res) => {
  ...
  try {
    ...
    if (Number.isFinite(maxNum)) {
      ...
      const recordFiles = [];
      if (fs.existsSync(nestedFile)) recordFiles.push(nestedFile);
      recordFiles.push(...listRecordFiles(targetDir).filter(f => f !== nestedFile));
    ...
    if (Number.isFinite(maxNum)) {
      const { targetDir } = nestedPathFromNumber('max', maxNum);
      try { fs.mkdirSync(targetDir, { recursive: true }); } catch (_) {}
      const recordPath = generateRecordFilePath(targetDir);
      try { fs.writeFileSync(recordPath, line); console.log('[LOG] max write:', recordPath); written.max = recordPath; } catch (e) { console.warn('[LOG] max write failed:', recordPath, e); }
    }
    if (Number.isFinite(minNum)) {
      const { targetDir } = nestedPathFromNumber('min', minNum);
      try { fs.mkdirSync(targetDir, { recursive: true }); } catch (_) {}
      const recordPath = generateRecordFilePath(targetDir);
      try { fs.writeFileSync(recordPath, line); console.log('[LOG] min write:', recordPath); written.min = recordPath; } catch (e) { console.warn('[LOG] min write failed:', recordPath, e); }
    }
...
app.get('/api/log/by-max', (req, res) => {
  try {
    ...
    if (fs.existsSync(nestedFile)) candidateFiles.push(nestedFile);
    candidateFiles.push(...listRecordFiles(targetDir).filter(f => f !== nestedFile));
    
    let sourceFiles = candidateFiles;
    if (sourceFiles.length === 0) {
      sourceFiles = findAllLogFiles(baseDir, 'max', digits);
      if (sourceFiles.length === 0) {
    ...
    return res.json({ ok: true, params: { nb_max: maxNum }, files: sourceFiles.length, dir: baseDir, count: slice.length, items: slice });
  } catch (e) {
    return res.status(500).json({ ok: false, error: String(e) });
  }
});
...
app.get('/api/log/by-min', (req, res) => {
  try {
    ...
    let sourceFiles = candidateFiles;
    if (sourceFiles.length === 0) {
      sourceFiles = findAllLogFiles(baseDir, 'min', digits);
      if (sourceFiles.length === 0) {
    ...
    allItems.sort((a, b) => (b.t || 0) - (a.t || 0));
    const slice = allItems.slice(0, limit);
    return res.json({ ok: true, params: { nb_min: minNum }, files: sourceFiles.length, dir: baseDir, count: slice.length, items: slice });
  } catch (e) {
    return res.status(500).json({ ok: false, error: String(e) });
  }
});
  ...
  try {
    ...
    if (novelPrefix) {
      if (attributeText.startsWith(novelPrefix)) {
        // 이미 전체 경로 형태이므로 그대로 둡니다
      } else if (attributeText.length > 0) {
        attributeText = `${novelPrefix}${attributeText}`;
      } else {
        attributeText = `${novelPrefix}`;
      }
    }
    ...
    // 중복 체크 (속성 MAX 파일 기준)
    const { targetDir: checkDir, nestedFile: checkFile } = nestedPathFromNumber('max', attributeBitMax);
    ...
    if (Number.isFinite(attributeBitMax)) {
      ...
      const recordPath = generateRecordFilePath(targetDir);
      try { 
        fs.writeFileSync(recordPath, line, 'utf8'); 
        console.log('[Attribute] 속성 MAX 폴더에 저장:', recordPath); 
        written.attributeMax = recordPath; 
      } catch (e) { 
        console.warn('[Attribute] 속성 MAX 저장 실패:', recordPath, e); 
      }
    }
    ...
    if (Number.isFinite(attributeBitMin)) {
      ...
      try { 
        fs.writeFileSync(recordPath, line, 'utf8'); 
        console.log('[Attribute] 속성 MIN 폴더에 저장:', recordPath); 
        written.attributeMin = recordPath; 
      } catch (e) { 
        console.warn('[Attribute] 속성 MIN 저장 실패:', recordPath, e); 
      }
    }
...
app.post('/api/attributes/data/delete', (req, res) => {
  try {
    const { attributeBitMax, attributeBitMin, text: dataTextRaw, dataBitMax, dataBitMin } = req.body || {};
    const dataText = typeof dataTextRaw === 'string' ? dataTextRaw.trim() : '';
    ...
    const hasDataBits = Number.isFinite(dataBitMax) && Number.isFinite(dataBitMin);
    const hasDataText = dataText.length > 0;
      ...
      try {
        ...
        for (const line of lines) {
          try {
            ...
            let dataMatch = true;
            if (hasDataText) {
              const recordDataText = (parsed.data?.text || parsed.s || '').trim();
              dataMatch = recordDataText === dataText;
            } else if (hasDataBits) {
              const recordDataBitMax = parsed.data?.bitMax ?? parsed.max ?? null;
              const recordDataBitMin = parsed.data?.bitMin ?? parsed.min ?? null;
              dataMatch =
                Number.isFinite(recordDataBitMax) &&
                Number.isFinite(recordDataBitMin) &&
                recordDataBitMax === dataBitMax &&
                recordDataBitMin === dataBitMin;
            }
...
async function collectAllAttributes() {
        ...
        for (const line of lines) {
          try {
            ...
            if (parsed.attribute && parsed.attribute.text && parsed.attribute.bitMax !== undefined && parsed.attribute.bitMin !== undefined) {
              ...
              const key = `${attr.bitMax}_${attr.bitMin}_${attr.text || ''}`;
              if (!seen.has(key)) {
                seen.add(key);
  ...
  for (const [cellId, attr] of attributes) {
    try {
      ...
      logFiles.push(...listRecordFiles(targetDir).filter(f => f !== nestedFile));
        ...
        try {
          ...
          for (const line of lines) {
            try {
              ...
              if (
                parsed.attribute &&
                parsed.attribute.bitMax === attr.bitMax &&
                parsed.attribute.bitMin === attr.bitMin &&
                (parsed.attribute.text || '') === attr.text
              ) {
                count++;
              }
            } catch {}
...
app.post('/api/tests/reset-all', (req, res) => {
  try {
    const { confirm } = req.body || {};
    if (confirm !== 'RESET') {
      return res.status(400).json({ ok: false, error: 'confirm must be "RESET"' });
    }
    resetAllTestData();
    return res.json({ ok: true });
  } catch (error) {
    console.error('[Tests] reset-all error:', error);
    return res.status(500).json({ ok: false, error: String(error.message || error) });
  }
});

app.post('/api/tests/sample-data', async (req, res) => {
  try {
    const { novelTitle, chapters } = req.body || {};
    if (!novelTitle || !Array.isArray(chapters)) {
      return res.status(400).json({ ok: false, error: 'novelTitle and chapters array required' });
    }
    ...
    for (const chapter of chapters) {
      if (!chapter || !chapter.label || !Array.isArray(chapter.sections)) continue;
      const chapterLabel = chapter.label;
      const chapterBits = calculateBitValues(chapterLabel);
      const chapterInfo = {
        number: chapter?.info?.number || null,
        title: chapter?.info?.title || null,
        description: chapter?.info?.description || null
      };
      ...
      for (const section of chapter.sections) {
        ...
        try {
          const response = await fetch(`http://127.0.0.1:${PORT}/api/attributes/data`, {
            method: 'POST',
    ...
    console.error('[Tests] sample-data error:', error);
    return res.status(500).json({ ok: false, error: String(error.message || error) });
  }
});

app.get('/api/tests/records', (req, res) => {
  try {
    const novelTitle = req.query.novelTitle ? String(req.query.novelTitle).trim() : '';
      ...
      for (const filePath of files) {
        ...
        lines.forEach((line, index) => {
          try {
            ...
            const attrText = record.attribute?.text;
            if (!attrText) return;
            if (novelTitle && record.novel?.title !== novelTitle) return;
            const dataText = record.data?.text || record.s || '';
            const normalizedPath = filePath.replace(/\\/g, '/');
            const key = `${attrText}::${dataText}::${normalizedPath}::${record.timestamp || index}`;
            if (unique.has(key)) return;
            unique.set(key, {
              id: key,
              novelTitle: record.novel?.title || null,
              attributeText: attrText,
              attributeBits: {
                max: record.attribute?.bitMax ?? null,
                min: record.attribute?.bitMin ?? null
              },
              dataText,
              dataBits: {
                max: record.data?.bitMax ?? record.max ?? null,
                min: record.data?.bitMin ?? record.min ?? null
              },
              chapter: record.chapter || null,
              timestamp: record.timestamp || null,
              createdAt: record.t || null,
              source: { file: filePath, line: index + 1 }
            });
          } catch {}
        });
      }
    ...
    const items = Array.from(unique.values())
      .sort((a, b) => (a.attributeText || '').localeCompare(b.attributeText || '', 'ko', { numeric: true }))
      .slice(0, limit);

    return res.json({ ok: true, count: items.length, items });
  } catch (error) {
    console.error('[Tests] records error:', error);
    return res.status(500).json({ ok: false, error: String(error.message || error) });
  }
});
