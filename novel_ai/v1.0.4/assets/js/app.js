document.addEventListener('DOMContentLoaded', () => {
    ...
    let isLoadingServerNovels = false; // 로딩 중 플래그 (중복 호출 방지)
    ...
    async function loadServerNovels() {
        // 이미 로딩 중이면 중복 호출 방지
        if (isLoadingServerNovels) {
            addLog('warn', '[BIT 기반 소설 목록] 이미 로딩 중입니다.');
            return;
        }
        
        isLoadingServerNovels = true;
        try {
            ...
            // renderNovelTree() 호출 제거 - 렌더링은 renderConversationList()에서 처리
        } catch (error) {
            // 상세 오류 정보 수집
            const errorDetails = {
            ...
            // 기본 오류 메시지
            addLog('error', `[BIT 기반 소설 목록] 로드 오류: ${errorDetails.message}`);
            addLog('error', `[BIT 기반 소설 목록] 오류 타입: ${errorDetails.name} (${errorDetails.type})`);
            
            // 스택 트레이스를 각 줄별로 로그에 추가 (처음 15줄만)
            if (error.stack) {
                const stackLines = error.stack.split('\n').slice(0, 15);
                addLog('error', `[BIT 기반 소설 목록] 스택 트레이스 (${stackLines.length}줄):`);
                stackLines.forEach((line, index) => {
                    const trimmedLine = line.trim();
                    if (trimmedLine) {
                        addLog('error', `  [${index + 1}] ${trimmedLine}`);
                    }
                });
            } else {
                addLog('error', '[BIT 기반 소설 목록] 스택 정보 없음');
            }
            
            // 콘솔에도 전체 오류 출력 (디버깅용)
            console.error('[BIT 기반 소설 목록] 상세 오류 정보:', {
                message: errorDetails.message,
                name: errorDetails.name,
                type: errorDetails.type,
                stack: error.stack
            });
            console.error('[BIT 기반 소설 목록] 원본 오류 객체:', error);
            
            serverNovels = [];
        } finally {
            isLoadingServerNovels = false;
        }
    }
    ...
    async function saveChaptersToNBData(novelTitle, chapters) {
        ...
        // 각 챕터 저장
        for (const chapter of chapters) {
            // 챕터 제목이 이미 "챕터 X: ..." 형식이면 그대로 사용, 아니면 구성
            let chapterText;
            if (chapter.title && /^챕터\s*\d+/.test(chapter.title)) {
                chapterText = chapter.title;
            } else if (chapter.number && chapter.title) {
                chapterText = `챕터 ${chapter.number}: ${chapter.title}`;
            } else if (chapter.title) {
                chapterText = chapter.title;
            } else if (chapter.number) {
                chapterText = `챕터 ${chapter.number}`;
            } else {
                continue;
            }
            ...
            // 전체 attributeText 구성
            const fullAttributeText = `${novelTitle} → ${chapterText}`;
            
            // attributeText의 BIT 값 계산 (소설 제목만이 아닌 전체 텍스트)
            const attributeBits = calculateBitValues(fullAttributeText);
            if (!attributeBits.max || !attributeBits.min) {
                addLog('warn', `[N/B DATA] 속성 BIT 계산 실패: ${fullAttributeText}`);
                continue;
            }
            ...
            try {
                ...
                const response = await fetch(url, {
                    ...
                    body: JSON.stringify({
                        ...
                        novelTitle: novelTitle,
                        novelTitleBitMax: titleBits.max,
                        novelTitleBitMin: titleBits.min,
                        novel: {
                            title: novelTitle,
                            chapter: chapter
                        },
...
좌측 Novel AI에 표시된 속성과 데이터를 참조하여 더 정확하고 일관성 있는 응답을 제공하세요.`;
            ...
            if (errorMsg.includes('API key') || errorMsg.includes('key')) {
                appendMessage('assistant', `❌ API 키가 설정되지 않았습니다.\n\n헤더의 "🔑 API 키" 버튼을 클릭하여 설정해주세요.`, false, 'gpt');
                
                // 메시지 추가 후 버튼 추가
                setTimeout(() => {
                    const messages = $mainContent.querySelectorAll('.message.assistant');
    ...
    const $logDiagram = document.getElementById('logDiagram');
    const $logDiagramItems = $logDiagram ? Array.from($logDiagram.querySelectorAll('.log-diagram-item')) : [];
    const $logDetailsPanel = document.getElementById('logDetailsPanel');
    const $logDetailsTitle = document.getElementById('logDetailsTitle');
    const $logDetailsCount = document.getElementById('logDetailsCount');
    const $logDetailsCloseBtn = document.getElementById('logDetailsCloseBtn');
    const MAX_LOG_ENTRIES = 50;
    const LOG_STORAGE_KEY = 'novel_ai_log_history';
    const LOG_TYPE_LABELS = { all: '전체', info: '정보', message: '로그', warn: '경고', error: '오류' };
    const KNOWN_LOG_TYPES = new Set(['info', 'message', 'warn', 'error']);
    const logHistory = [];
    let activeLogFilter = null;
    
    function createLogEntryElement(record) {
        const entry = document.createElement('div');
        entry.className = 'log-entry';
        entry.setAttribute('data-type', record.type);
        entry.setAttribute('data-timestamp', record.timestamp);
        entry.setAttribute('data-full-message', record.message);
        entry.dataset.createdAt = String(record.createdAt || Date.now());
        ...
        const displayText = record.message.length > 100 ? record.message.substring(0, 97) + '...' : record.message;
        ...
        entry.addEventListener('click', () => {
            ...
            pre.textContent = record.message;
    ...
    function loadLogHistoryFromStorage() {
        try {
            ...
            return parsed
                .map(item => ({
                    type: KNOWN_LOG_TYPES.has(item.type) ? item.type : 'message',
    ...
    function renderAllLogs() {
        ...
        const currentFilter = activeLogFilter || 'all';
        $logContainer.innerHTML = '';
        logHistory.forEach(record => {
            const entry = createLogEntryElement(record);
            $logContainer.appendChild(entry);
        });
        filterLogEntries(currentFilter);
    }
    ...
    function addLog(type, ...args) {
        ...
        const normalizedType = KNOWN_LOG_TYPES.has(type) ? type : 'message';
        const record = {
            type: normalizedType,
            timestamp: ts,
            message: full,
        ...
        logHistory.unshift(record);
        if (logHistory.length > MAX_LOG_ENTRIES) {
            logHistory.length = MAX_LOG_ENTRIES;
            while ($logContainer.children.length > MAX_LOG_ENTRIES) {
                $logContainer.removeChild($logContainer.lastElementChild);
            }
        }

        filterLogEntries(activeLogFilter || 'all');
        persistLogHistory();
        updateLogDiagram();
    }

    function filterLogEntries(type) {
        if (!$logContainer) return 0;
        const normalized = type && type !== 'all' ? type : 'all';
        const entries = Array.from($logContainer.querySelectorAll('.log-entry'));
        let visibleCount = 0;
        entries.forEach(entry => {
            const entryType = entry.getAttribute('data-type') || 'message';
            const shouldShow = normalized === 'all' || entryType === normalized;
    ...
    function updateLogDetailsHeader(type, count) {
        if ($logDetailsTitle) {
            const label = LOG_TYPE_LABELS[type] || (type && type !== 'all' ? type : '전체');
            $logDetailsTitle.textContent = `${label} 로그`;
        }
        if ($logDetailsCount) {
            $logDetailsCount.textContent = typeof count === 'number' ? `${count}건` : '';
        }
    }

    function showLogDetails(type) {
        if (!$logDetailsPanel) return;
        const normalized = type && type !== 'all' ? type : 'all';
        activeLogFilter = normalized;
        const visibleCount = filterLogEntries(normalized);
        $logDetailsPanel.classList.remove('hidden');
        updateLogDetailsHeader(normalized, visibleCount);
    }
    ...
    function closeLogDetails() {
        ...
        if ($logDetailsTitle) {
            $logDetailsTitle.textContent = '로그 상세';
        }
        if ($logDetailsCount) {
            $logDetailsCount.textContent = '';
        }
    }

    function refreshLogDetails() {
        if (!activeLogFilter || !$logDetailsPanel || $logDetailsPanel.classList.contains('hidden')) {
            return;
        }
        const visibleCount = filterLogEntries(activeLogFilter);
        updateLogDetailsHeader(activeLogFilter, visibleCount);
    }

    function updateLogDiagram() {
        if (!$logDiagramItems.length) {
            refreshLogDetails();
            return;
        }
        ...
        for (const log of logHistory) {
            const key = KNOWN_LOG_TYPES.has(log.type) ? log.type : 'message';
            totals[key] = (totals[key] || 0) + 1;
        }
        const totalCount = logHistory.length;

        $logDiagramItems.forEach(item => {
            const type = item.dataset.type || 'all';
            const count = type === 'all' ? totalCount : (totals[type] || 0);
            ...
            if (percentageEl) {
                percentageEl.textContent = totalCount ? `${percentage}%` : '0%';
            }
        ...
        if (!totalCount && $logDetailsPanel && !$logDetailsPanel.classList.contains('hidden')) {
            closeLogDetails();
        } else {
            refreshLogDetails();
        }
    ...
    if ($logDetailsCloseBtn) {
        $logDetailsCloseBtn.addEventListener('click', () => {
            closeLogDetails();
        });
    }

    const persistedLogs = loadLogHistoryFromStorage();
    if (persistedLogs.length) {
        logHistory.length = 0;
        persistedLogs.forEach(log => logHistory.push(log));
        renderAllLogs();
    } else {
        filterLogEntries('all');
    }
    ...
    window.addEventListener('storage', (event) => {
        if (event.key === LOG_STORAGE_KEY) {
            const syncedLogs = loadLogHistoryFromStorage();
            logHistory.length = 0;
            syncedLogs.forEach(log => logHistory.push(log));
            renderAllLogs();
            updateLogDiagram();
        }
    });
    ...
    if ($logWrapper) {
        ...
        if ($topLogResizeHandle && $logWrapper) {
            ...
            $topLogResizeHandle.addEventListener('mousedown', (e) => {
                ...
                $logWrapper.classList.add('is-resizing');
                const previousCursor = document.body.style.cursor;
                const previousUserSelect = document.body.style.userSelect;
                document.body.dataset.prevCursor = previousCursor || '';
                document.body.dataset.prevUserSelect = previousUserSelect || '';
                ...
                const stopResize = () => {
                    ...
                    $logWrapper.classList.remove('is-resizing');
    ...
    async function checkApiKeyStatus() {
        try {
            const url = getServerUrl('/api/gpt/key');
            const response = await fetch(url);
            if (response.ok) {
                const data = await response.json();
                if (data.ok && data.hasKey) {
                    ...
                    $apiKeyBtn.classList.remove('btn-outline-danger');
                    $apiKeyBtn.classList.add('btn-outline-secondary');
                    addLog('info', '[API 키] 상태 확인: 설정됨');
                } else {
                    $apiKeyStatus.textContent = '⚠ 미설정';
                    $apiKeyStatus.className = 'api-key-status text-warning';
                    $apiKeyBtn.classList.remove('btn-outline-secondary');
                    $apiKeyBtn.classList.add('btn-outline-danger');
                    
                    // 디버깅 정보 로그
                    if (data.fileExists) {
                        addLog('warn', `[API 키] 파일은 존재하지만 키가 비어있습니다. 파일 경로: ${data.filePath || '알 수 없음'}`);
                    } else {
                        addLog('warn', `[API 키] 파일이 존재하지 않습니다. 파일 경로: ${data.filePath || '알 수 없음'}`);
                    }
                }
            } else {
                $apiKeyStatus.textContent = '? 확인 불가';
                $apiKeyStatus.className = 'api-key-status text-muted';
                addLog('error', '[API 키] 상태 확인 실패: 서버 응답 오류');
            }
        } catch (error) {
            $apiKeyStatus.textContent = '? 오류';
            $apiKeyStatus.className = 'api-key-status text-danger';
            console.error('API 키 상태 확인 오류:', error);
            addLog('error', `[API 키] 상태 확인 오류: ${error.message}`);
        }
    }
    ...
    if ($apiKeyBtn && $apiKeyModal) {
        $apiKeyBtn.addEventListener('click', () => {
            ...
            if ($apiKeyInput) {
                $apiKeyInput.value = '';
                $apiKeyModalStatus.style.display = 'none';
            }
        });
    }
    ...
    if ($saveApiKeyBtn && $apiKeyInput) {
        $saveApiKeyBtn.addEventListener('click', async () => {
            ...
            if (!apiKey) {
                $apiKeyModalStatus.textContent = 'API 키를 입력해주세요.';
                $apiKeyModalStatus.className = 'alert alert-warning';
                $apiKeyModalStatus.style.display = 'block';
                return;
            }
            
            if (!apiKey.startsWith('sk-')) {
                $apiKeyModalStatus.textContent = '유효한 OpenAI API 키 형식이 아닙니다. (sk-로 시작해야 합니다)';
                $apiKeyModalStatus.className = 'alert alert-warning';
                $apiKeyModalStatus.style.display = 'block';
                return;
            }
            ...
            try {
                ...
                if (response.ok) {
                    const data = await response.json();
                    if (data.ok) {
                        $apiKeyModalStatus.textContent = '✓ API 키가 성공적으로 저장되었습니다.';
                        $apiKeyModalStatus.className = 'alert alert-success';
                        $apiKeyModalStatus.style.display = 'block';
                ...
                $apiKeyModalStatus.textContent = `✗ 저장 실패: ${error.message}`;
                $apiKeyModalStatus.className = 'alert alert-danger';
                $apiKeyModalStatus.style.display = 'block';
                addLog('error', `[API 키] 저장 오류: ${error.message}`);
            } finally {
    ...
    // 모달이 닫힐 때 입력창 초기화
    if ($apiKeyModal) {
        $apiKeyModal.addEventListener('hidden.bs.modal', () => {
            if ($apiKeyInput) {
                $apiKeyInput.value = '';
                $apiKeyModalStatus.style.display = 'none';
            }
